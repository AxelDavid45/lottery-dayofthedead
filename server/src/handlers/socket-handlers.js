  ...
  io.on('connection', (socket) => {
    ...
    socket.on('room:create', async (data) => {
      try {
        ...
        if (name.length > 20) {
          socket.emit('error', { code: 'NAME_TOO_LONG', message: 'Name must be 20 characters or less' });
          return;
        }
    ...
    socket.on('room:join', async (data) => {
      try {
        ...
        if (!roomCode || !name) {
          socket.emit('error', { code: 'MISSING_DATA', message: 'Room code and name are required' });
          return;
        }
        ...
        const room = roomManager.joinRoom(roomCode.toUpperCase(), socket.id, name.trim());
    ...
    socket.on('game:start', async (data) => {
      try {
        ...
        if (room.status !== 'WAITING') {
          socket.emit('error', { code: 'GAME_ALREADY_STARTED', message: 'Game is already running' });
          return;
        }

        // Game start logic will be implemented in later tasks
        console.log(`Game start requested for room ${roomCode} by host ${socket.id}`);
        socket.emit('error', { code: 'NOT_IMPLEMENTED', message: 'Game start will be implemented in next tasks' });
